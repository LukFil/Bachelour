...
createPvalTab <- function(files,postfix="",pvaluelist=c(0.001,0.01,0.05,0.1),
  ...
  for(i in 1:length(files)){
	...
	for (count in 1:length(pvaluelist)){
		...
		up <- sum((tab[,namePVal] < pvaluelist[count]) & (tab[,nameLogFC] >= 0),na.rm=TRUE)
		down <- sum((tab[,namePVal] < pvaluelist[count]) & (tab[,nameLogFC] < 0),na.rm=TRUE)
	...
	for (count in 1:length(adjpvaluelist)){
		...
		adjUp <- sum((tab[,nameAdjPVal] < adjpvaluelist[count]) & (tab[,nameLogFC] >= 0),na.rm=TRUE)
		adjDown <- sum((tab[,nameAdjPVal] < adjpvaluelist[count]) & (tab[,nameLogFC] < 0),na.rm=TRUE)
	...
	for (count in 1:length(foldchangelist)){
		...
		FCUp <- length(tab[(tab[,nameFC] >= foldchangelist[count]) & (!is.na(tab[,nameFC])),1])
		FCDown <- length(tab[(tab[,nameFC] <= (-foldchangelist[count])) & (!is.na(tab[,nameFC])),1])
